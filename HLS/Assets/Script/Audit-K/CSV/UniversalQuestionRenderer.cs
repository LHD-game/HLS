...
public class UniversalQuestionRenderer : MonoBehaviour
{
    ...
    private int currentQuestionIndex = 7; // 첫 번째 질문 인덱스 (7로 시작)
    private int lastAuditKQuestionIndex = 16; // Audit-K 마지막 질문 인덱스
        ...
        for (int i = 1; i <= 5; i++)
        {
            ...
            if (!string.IsNullOrEmpty(choiceText))
            {
                ...
                if (newButton != null)
                {
                    ...
                    int score = i - 1; // 0~4점 처리
                    if (currentQuestionIndex == 16 && (i == 3 || i == 4 || i == 5))
                    {
                        if (i == 3) score = 3; // 특정 문항의 특수 점수 처리
                    }
    ...
    {
        if (currentQuestionIndex < lastAuditKQuestionIndex) // Audit-K 마지막 질문까지만 처리
        {
        ...
        {
            Debug.Log("더 이상 질문이 없습니다."); // Audit-K 범위 끝났을 때 처리
        }
    }

    public void PreviousQuestions()
    {
        if (currentQuestionIndex > 7) // 7번 질문부터 시작
        {
            currentQuestionIndex--;
            RenderQuestions();
        }
