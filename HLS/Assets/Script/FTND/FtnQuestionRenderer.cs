...
public class FtnQuestionRenderer : MonoBehaviour
{
    ...
    public FtnScoreManager scoreManager;
    private FtnCsvReader csvReader;
    private int currentQuestionIndex = 3; // FTND 첫 번째 질문 인덱스 (데이터가 3번째 행부터 시작한다고 가정)
    private int lastFTNDQuestionIndex = 8; // FTND 마지막 질문 인덱스 (총 6개의 질문)
    ...
    public void RenderQuestions()
    {
        ...
        // 버튼을 동적으로 생성
        for (int i = 1; i <= 4; i++) // FTND는 최대 4개의 선택지가 있다고 가정
        {
    ...
    {
        if (currentQuestionIndex < lastFTNDQuestionIndex) // FTND 마지막 질문까지만 처리
        {
        ...
        {
            Debug.Log("더 이상 질문이 없습니다."); // FTND 범위 끝났을 때 처리
        }
    }

    public void PreviousQuestions()
    {
        if (currentQuestionIndex > 3) // FTND 첫 번째 질문 인덱스
        {
            currentQuestionIndex--;
            RenderQuestions();
        }
